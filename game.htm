...
<body>
    ...
    <script type="module">
        ...
        const obstacleSpeed = 0.03; // A constant forward speed for obstacles
        let driveSpeed = 0;
        let currentSpeed = 0;
        ...
        function createBuildingTexture() {
            ...
            context.fillStyle = `hsl(${Math.random() * 360}, 10%, 60%)`;
            ...
            const gap = 16;
            context.fillStyle = '#222233'; // Darker for windows

            for (let x = gap; x < textureWidth - gap; x += windowWidth + gap) {
                for (let y = gap; y < textureHeight - gap; y += windowHeight + gap) {
        ...
        function setupEnvironment() {
            ...
            // Add road barriers
            const barrierGeo = new THREE.BoxGeometry(0.5, 1, roadLength * 2);
            const barrierLeft = new THREE.Mesh(barrierGeo, barrierMaterial);
            barrierLeft.position.set(-roadWidth / 2 - kerbWidth - 0.25, 0.5, 0);
            barrierLeft.castShadow = true;
            barrierLeft.receiveShadow = true;
            scene.add(barrierLeft);
            roadBarriers.push(barrierLeft);

            const barrierRight = new THREE.Mesh(barrierGeo, barrierMaterial);
            barrierRight.position.set(roadWidth / 2 + kerbWidth + 0.25, 0.5, 0);
            barrierRight.castShadow = true;
            barrierRight.receiveShadow = true;
            scene.add(barrierRight);
            roadBarriers.push(barrierRight);
        ...
        function generateBuildings(count) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                for (let side = -1; side <= 1; side += 2) {
                    ...
                    const material = new THREE.MeshLambertMaterial({ map: buildingTexture.clone() });
                    // Randomize texture offset to make each building look unique
                    material.map.repeat.set(width / 10, height / 10);
                    material.map.offset.set(Math.random() * (1 - material.map.repeat.x), Math.random() * (1 - material.map.repeat.y));
        ...
        function generateTraffic(count) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                // Assign a unique speed to each car
                // Now they will always move forward relative to the player
                enemyCar.userData.speed = Math.random() * 0.2 + 0.1;
                enemyCar.userData.relativeZ = 0;
        ...
        function recycleObjects() {
            ...
            buildings.forEach(building => {
                if (building.position.z < -roadLength) {
                    ...
                    // Reset texture offset for recycled building
                    if (building.material && building.material.map) {
                        building.material.map.repeat.set(width / 10, height / 10);
                        building.material.map.offset.set(Math.random() * (1 - building.material.map.repeat.x), Math.random() * (1 - building.material.map.repeat.y));
                    }
                }
            });
            ...
            enemyCars.forEach(car => {
                // If the car is too far back, recycle it
                if (car.position.z < -roadLength / 2) {
                    ...
                    // Give it a new random speed for the next run, always a positive value
                    car.userData.speed = Math.random() * 0.2 + 0.1;
                }
            });
            ...
            roadBarriers.forEach(barrier => {
                if (barrier.position.z < -roadLength) {
                    barrier.position.z += roadLength * 2;
                }
            });
        }
        ...
        function resetGame() {
            ...
            generateTraffic(8);
            generateTreesAndFlowers(50);
            roadBarriers.forEach(b => {b.position.z = 0}); // Reset barriers position
        ...
        function animate() {
            ...
            // Move the world based on the player's speed
            const worldMoveSpeed = Math.abs(driveSpeed);
            const obstacleMoveSpeed = worldMoveSpeed + obstacleSpeed;

            roadLines.forEach(obj => { if(obj) obj.position.z -= worldMoveSpeed; });
            buildings.forEach(obj => { if(obj) obj.position.z -= worldMoveSpeed; });
            streetLights.forEach(obj => { if(obj) obj.position.z -= worldMoveSpeed; });
            points.forEach(obj => { if(obj) obj.position.z -= worldMoveSpeed; });
            roadBarriers.forEach(obj => { if(obj) obj.position.z -= worldMoveSpeed; });
            trees.forEach(obj => { if(obj) obj.position.z -= worldMoveSpeed; });
            flowers.forEach(obj => { if(obj) obj.position.z -= worldMoveSpeed; });

            // Move each enemy car forward, always
            enemyCars.forEach(car => {
                if (car) {
                    car.position.z -= obstacleMoveSpeed;
                }
            });
